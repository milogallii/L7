...
The subsequent discussion will address the necessity for "software-defined networking" and the innovative technologies underpinning network administration, EBPF and AF\_XDP.
...
Since its inception, the protocol has evolved significantly to accommodate advancements in technology and the expanding needs of the naval and gps fields.

\begin{figure}[H]
	\centering
    \includegraphics[scale=0.35]{images/nmea_timeline.png}
...
Although the protocol stipulates the use of RS-422 for electrical transport, a de facto standard has emerged in which sentences are \textbf{encapsulated in UDP datagrams} and \textbf{transmitted over IP networks} taking the name of LightWeight Ethernet (LWE).
...
To date, the research community has conducted only a limited number of studies on the security of network marine protocols. Moreover, there is a paucity of well documented research on risk analysis for this type of communication \cite{marine_network_protocols}.
...
\begin{tabularx}{\textwidth}{|>{\hsize=0.75\hsize}X|>{\hsize=1.25\hsize}X|}
  ...
  Attack & Methodology \\ \hline
    Denial of Service & NMEA 0183 supports a baud rate of 4800 (9600 bits/s). While being a ethernet specific this makes a DoS attack potentially effective against any devices using this protocol depending on the means through which data is transferred to an application\\ \hline
...
\begin{figure}[H]
	\centering
    \includegraphics[scale=0.15]{images/nmea2000_can.png}
...
\begin{figure}[H]
	\centering
    \includegraphics[scale=0.35]{images/sdn.png}
...
\section{Next generation networking}
...
\begin{itemize}
    ...
    \item\textbf{Kernel Interrupts} :  They are signals used to stop the CPU from what it is doing and work on the interrupter’s part instead. They are divided into two kinds, top-half (simple and fast tasks) and bottom-half (complex and heavy tasks), differentiated by computational cost and calling reasons.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Network packet arrives at the NIC
    \item DMA writes the packet to the RX Ring buffer
    \item NIC interfaces with the driver which tells the CPU that new traffic is ready to be processed using Hardware Interrupt ( top-half, since it is a time critical and fast task )
    \item After the first Hardware interrupt, the Interrupt Handler masks it, and instead the driver adopts the use of Software Interrupt ( bottom-half, since the packet processing is a heavier task ) which is computationally cheaper for the CPU
    \item The Software Interrupt invokes the New-API subsystem, which calls the NIC Driver’s Polling function
    \item CPU processes the incoming packets
    \item Some time after the Software Interrupt budget runs out and the CPU moves on to the next task
\end{enumerate}

\begin{figure}[H]
	\centering
    \includegraphics[scale=0.35]{images/Linux_Network_stack.png}
...
\begin{figure}[H]
	\centering
    \includegraphics[scale=0.12]{images/ebpf_structure.png}
...
\begin{figure}[H]
	\centering
    \includegraphics[scale=0.3]{images/xdp_afxdp.png}
...
\begin{figure}[H]
	\centering
    \includegraphics[scale=0.3]{images/af_xdp_socket_structure.png}
...
Many of the optimizations brought to the networking hardware and drivers are set with the goal to achieve higher rates, in terms of bits per second (bps) or packets per second (pps). While optimizing for throughput, it is trivial to assume that some components may induce higher latency especially at lower packet rates.
