...
The integration of these distinctive characteristics and the language's rich type system facilitated the simulation of the ship and its components. The life cycle and the interactions within the various devices were collapsed within just two files constituting the project's fundamental structural elements.\\
...
It is noteworthy that the ship itself and each component are implemented as a Struct product type.
Considering that creating the system's simulation is as simple as initializing an array of variables, as it's possible to state looking at the program's main function\\
