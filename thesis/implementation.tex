...
\subsection{Ship Abstraction's Layout}

Leveraging these unique features and the rich type system we were able to simulate the ship components and their life cycle within just two files, forced by project layout's costraints. \\
...
In addition to static data, such as the MAC and IP addresses, which are contained within the struct, there exist variables that oversee the AF\_XDP socket bound to the component's NIC. This approach enables a high degree of flexibility by leveraging the impl pattern, which facilitates the definition of methods tailored to the component's specific functionality. A prime illustration of this methodology is evident in the manner in which each component administers the application of the network policy. \\
...
The components' networking capabilities, which leverage the AF\_XDP technology, were developed using the \textit{xdrippi} crate. This library provides low-level accessibility to the socket's main components through fairly simple APIs. The socket's initialization and setup is accomplished in a concise number of lines of code, as is the management of the ring buffers and UMEM. It is noteworthy that the crate's implementation utilizes \textit{glibc} primitives for socket creation and management, which may potentially lead to unsafe memory management. However, as previously mentioned, the use of unsafe blocks ensures the security and reliability of this implementation choice.\\
...
Rust's strong pattern matching capabilities were leveraged as well.
Network packets parsing became a trivial task  while retaining the benefits of low level checking thanks to zero-cost abstractions as well.\\
