...
% correct
To better explain the point made above, all the features described can be found in other programming languages but not all at the same time. Furthermore key aspects that contribute to make Rust the best choice for this implementation are :

\begin{itemize}
    \item\textbf{Algebraic Type System}, granting product and sum types that mixed with a strong pattern matching paradigm allows us to destructure and handle easily complex data aggregations that we could find, as in our case, in simulations with high level of structural detail
    \item\textbf{Data Ownership}, a new way of handling variables which assures us that data throughout the execution of our program is located and modified exactly where it should deleting completely the possibility of ambiguous memory behaviours
    \item\textbf{Unsafe block declarations}, this feature enables the raw access to memory that we would find in other low level languages like C without throwing away safety features intrinsic to to language.
    If a memory error occurs finding the root cause is immediate and low level hardware access is allowed in a monitored enviroment whose bounds are defined by the developer
\end{itemize}

%end-correct
% talk about these features in the code with snippets
