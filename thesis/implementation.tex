...
The policy is applied to the packets sent by the component with the desired level of granularity.
In the present implementation, the UDP packet undergoes parsing and is identified as a NMEA message if it conforms to the syntax rules of the protocol. Otherwise, it is handled as a standard UDP message.
...
The components' networking capabilities that leverage the AF\_XDP technology were developed using the \textit{xdrippi} crate which provides low level accessibility to the socket's main components thorugh fairly simple APIs. The socket's initialization and set up is performed in a few lines of code.


%correct
Another non-trivial feature to implement has been the policy marshalling.
The policy software market is saturated with infrastructure and application dependent programs whose application to legacy systems might imply changes to currently adopted standards.
The goal was to create a zero-dependecy and enviroment agnostic policy system, formalizing network communications without adding computational overhead.
The most reasonable choice was selecting the most simple configuration language letting the switch software integrate it into its process.

TOML files are the most popular choice for this kind of purpose due to their simplicity, readability, and flexibility. We decided to leverage these great features creating a components' configuration file of immediate understanding that doesn't lack of expressiveness and modularity\\
...
As it is possible to notice above the TOML file does not only define the policy rules, through a list of allowed send and receive messages, but also the entire structure of the simulation.
Adding a component to our OT system is as simple as adding a line of configuration with basic networking informations. The marshaling procedure is extremely easy as well thanks to Rust's "toml" crate, which allowed us to abstract the entirety of the file into an efficient Hash-Map data structure with few lines of code.\\
